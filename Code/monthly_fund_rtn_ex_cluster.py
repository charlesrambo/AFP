...
# Load clusters
clusters =  pd.read_csv(path + "\\Data\\Clusters\\clusters_cut_" + str(cut) + "_linkage_complete.csv")

# Rename columns
clusters.rename({'MAIN_KEY':'gvkey1'}, axis = 1, inplace = True)

# Merge clusters and hoberg
hoberg = hoberg.merge(clusters, on = ['year', 'gvkey1'])

# Rename columns
clusters.rename({'gvkey1':'gvkey2', 'cluster':'cluster2'}, axis = 1, inplace = True)

# Merge clusts with hoberg again
hoberg = hoberg.merge(clusters, on = ['year', 'gvkey2'])

del clusters

# Only consider hoberg values where cluster not equal to cluster2
hoberg = hoberg.loc[hoberg['cluster'] != hoberg['cluster2'], :]

# Drop cluster columns
hoberg.drop(['cluster', 'cluster2'], axis = 1, inplace = True)
...
# Convert to pandas
fundamentals.to_csv(path + '\\Data\\fundamental_mods\\monthly_fund_rtn_ex_cluster_cut_' + str(cut) + '.csv', index = False)
