...
# Load needed columns from monthly stock data
...
columns.drop_duplicates(inplace = True)
...
# Only consider observations with score greater than cut
hoberg = hoberg.loc[hoberg['score'] > cut, :]

# Calculate sum of scores
hoberg['sum'] = hoberg.groupby(['gvkey1', 'year'])['score'].transform('sum')

# Divide score by sum
hoberg['score'] = hoberg['score']/hoberg['sum']

# Drop score column
hoberg.drop('sum', axis = 1, inplace = True)
...
# Multiply RETURN2 by score
fundamentals['RETURN2'] = fundamentals['score'] * fundamentals['RETURN2']

# Drop score
fundamentals.drop('score', axis = 1, inplace = True)
...
# Convert to pandas
fundamentals.to_csv(path + '\\Data\\fundamental_mods\\weekly_fund_rtn_score_wt_cut_' + str(cut) + '.csv', index = False)
