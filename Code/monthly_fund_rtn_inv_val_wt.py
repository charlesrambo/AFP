...
stocks['inv_ME_lag'] = 1/stocks.groupby('DW_INSTRUMENT_ID')['ME'].shift(1)
...
# Record 75% quantile for each month
fundamentals['75'] = stocks[['DATE', 'inv_ME_lag']].groupby('DATE')['inv_ME_lag'].quantile(0.75)

# Replace missing ME_lags with 75-percentile value
fundamentals.loc[fundamentals['inv_ME_lag'].isna(), 'inv_ME_lag'] = fundamentals.loc[fundamentals['inv_ME_lag'].isna(), '75']

# Drop values with missing instrument ID, gvkey, or return
fundamentals.dropna(subset = ['DW_INSTRUMENT_ID', 'RETURN', 'inv_ME_lag'], axis = 0, how = 'any', inplace = True)

# Drop 75 and duplicates
fundamentals.drop('75', axis = 1, inplace = True)
...
rtn = fundamentals[['DATE', 'gvkey1', 'DW_INSTRUMENT_ID', 'RETURN', 'inv_ME_lag']]

# Drop ME_lag from fundamentals
fundamentals.drop('inv_ME_lag', axis = 1, inplace = True)
...
fundamentals['sum'] = fundamentals.groupby(['DATE', 'DW_INSTRUMENT_ID'])['inv_ME_lag'].transform('sum')

# Divide ME_lag by sum
fundamentals['inv_ME_lag'] = fundamentals['inv_ME_lag']/fundamentals['sum']

# Multiply RETURN2 by ME_lag
fundamentals['RETURN2'] = fundamentals['RETURN2'] * fundamentals['inv_ME_lag']

# Compute sum of RETURN2 to find inverse vale-weighted returns
...
# Convert to pandas
fundamentals.to_csv(path + '\\Data\\fundamental_mods\\monthly_fund_rtn_inv_val_wt_cut_' + str(cut) + '.csv', index = False)
